...
[ExecuteAlways]
public class VirtualLayoutOwn : UIBehaviour
{
    ...
    protected override void OnEnable()
    {
        ...
        for (int i = 1; i < children.Length; i++)
        {
            ...
            if (deleted && lastDeleted != -1 && _virtualLayoutChildren.Count == lastDeleted + 1 && nichtHinzu)
            {
                deleted = false;
                nichtHinzu = false;
                continue;
            ...
            //if( lastDeleted != -1 && _virtualLayoutChildren.Count == lastDeleted+1 && nichtHinzu)
            //{
            //    nichtHinzu = false;
            //} else
            //{
    ...
    {
        _virtualLayoutChildren.RemoveAt(index);
        lastDeleted = index;
        deleted = true;
        //if (Application.isPlaying)
        //{
        //    Destroy(_virtualLayoutChildren[index].gameObject);
        //}
        //else
        //{
        //    DestroyImmediate(_virtualLayoutChildren[index].gameObject);
        //}
    }
    ...
    private void LateUpdate()
    {
        ...
        for (int i = 0; i < _virtualLayoutChildren.Count; i++)
        {
            ...
            if(rectChild == null || virtualChild == null) continue;
            if (Application.isPlaying)
            {
